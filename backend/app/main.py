...
app = FastAPI(
    ...
    description=(
        ...
        "and delivers the report to the provided inbox via **Brevo** (transactional email API).\n\n"
        "### Live base URL\n"
        "**Production:** `https://sellix-sales-insight-generator.onrender.com`\n\n"
        "**Local dev:** `http://localhost:8000`\n\n"
        "### Authentication\n"
        "No auth required for local/dev use. Set `GROQ_API_KEY` and `BREVO_API_KEY` in `backend/.env`.\n\n"
        "### Rate limit\n"
